...
tags: [graphql]
draft: false
images: ['/static/images/graphql-design/banner.jpg']
...
```graphql:query-myCompany.graphql
query {
  myCompany {
    owner {
      name
...
```graphql:query-myCompanyOwner.graphql
query {
  myCompany {
    name
  }
...
```graphql:query-companyOwner.graphql
query {
...
```graphql:query-myCompany-products.graphql
query {
  myCompany {
    products {
      owner {
...
```graphql:query-myProductOwners.graphql
query {
  myProductOwners {
    firstName
    lastName
...
```graphql:mutation-createUser.graphql
mutation {
...
```graphql:mutation-updateUserName.graphql
type Mutation {
...
```graphql:mutation-deleteUser.graphql
type Mutation {
...
```graphql:mutation-removeCustomer.graphql
mutation {
  removeCustomer(id: 1) {
    id
  }
...
```graphql:query-myThirdParties.graphql
enum AuthStatus {
  DENIED
  REQUESTED
}
...
```json:myThirdParties-errors.json
{
  "data": {
    "myThirdParties": [
      {
